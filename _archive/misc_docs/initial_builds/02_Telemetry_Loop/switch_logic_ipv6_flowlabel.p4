...
struct metadata_t {
    ...
    bit<1>  health_valid;
    bit<32> meter_color;
    bit<1>  is_aipp_control;      // Fix 2: IEEE 802.3br Express Traffic Flag
}
...
control MyIngress(inout headers_t hdr,
    ...
    apply {
        // Fix 2: IDENTIFY EXPRESS TRAFFIC (GPOP/AIPP Control Frames)
        // We use a reserved Traffic Class (0xFF) to trigger the Express Lane.
        // This ensures the pre-charge signal is never queued behind bulk data.
        if (hdr.ipv6.trafficClass == 0xFF) {
            meta.is_aipp_control = 1;
            standard_metadata.priority = 7; // IEEE 802.1Qbu/802.3br pre-emption
        }
