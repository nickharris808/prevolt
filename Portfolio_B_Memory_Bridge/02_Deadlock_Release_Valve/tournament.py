...
class FixedTTLAlgorithm(Algorithm):
    ...
    Simple but effective - breaks deadlock deterministically
    but may drop packets that could have been delivered.
    """
    
    @property
    def name(self) -> str:
        return "Fixed TTL (1ms)"
...
class AdaptiveTTLAlgorithm(Algorithm):
    ...
    TTL scales with local congestion:
    - Low congestion: longer TTL (patient)
    - High congestion: shorter TTL (aggressive)
    
    This minimizes collateral drops while ensuring rapid
    deadlock recovery.
    """
    
    @property
    def name(self) -> str:
        return "Adaptive TTL (Invention)"
    
    def run(self, scenario: Scenario, seed: int) -> Dict[str, float]:
        config = DeadlockConfig(
            **scenario.params,
            adaptive_ttl_base_us=500.0,
