...
def run_closed_loop(cfg: TelemetryConfig, telemetry_enabled: bool) -> dict:
    ...
    # State variables for 2nd order dynamics
    v = np.zeros_like(t_ms)
    v[0] = cfg.v_nominal
    v_dot = 0.0 # Velocity of voltage change

    # Telemetry / control pipeline
    # Note: control_delay is 2*RTT
    ...
    # Initialize health history buffer (max RTT jitter handled here)
    max_delay_steps = int((cfg.rtt_ms * 4) / cfg.dt_ms)
    health_history = [health[0]] * (max_delay_steps + 1)

    for k in range(1, n):
        # Switch applies control with RTT JITTER (Fix: Real-world randomness)
        # Randomly vary the effective RTT by +/- 20%
        jitter_ms = cfg.rtt_ms * np.random.uniform(-0.2, 0.2)
        effective_rtt = max(0.1, cfg.rtt_ms + jitter_ms)
        control_delay_steps = int((2 * effective_rtt) / cfg.dt_ms)
        ...
        # SECOND-ORDER DYNAMICS (Fix: Modeling LC Resonance)
        # Models the inductor (L) and capacitor (C) on the GPU board.
        # This adds an inertial component to voltage drops.
        excess = max(0.0, throughput[k] - cfg.base_rate_gbps)
        drop_accel = cfg.drop_gain_v_per_gbps * excess * 100.0 # Accel of drop
        recover_accel = ((cfg.v_nominal - v[k - 1]) / cfg.vrm_tau_ms) * 5.0
        
        # Damping term (prevents infinite oscillation)
        damping = -v_dot * 0.5
        
        v_accel = -drop_accel + recover_accel + damping
        v_dot += v_accel * cfg.dt_ms
        v[k] = v[k - 1] + v_dot * cfg.dt_ms
